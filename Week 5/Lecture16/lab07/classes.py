...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        assert len(opponent.deck.cards)>3 and len(opponent.hand)>3, 'Not enough cards!'
        for i in range(3):
            opponent.hand.pop(i)
            opponent.draw()
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        temp = opponent_card.attack
        opponent_card.attack  = opponent_card.defense
        opponent_card.defense = temp
...
class ProfessorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        opp_cards_copy = opponent.deck.cards[:]
        for card in player.deck.cards:
            card.attack += opponent_card.attack
            card.defense += opponent_card.defense
        count = 0
        for idx in range(orig_opponent_deck_length):
            opp_card = opp_cards_copy[idx]
            if opp_card.attack==opponent_card.attack or opp_card.defense==opponent_card.defense:
                opponent.deck.cards.pop(idx-count)
                count+=1
